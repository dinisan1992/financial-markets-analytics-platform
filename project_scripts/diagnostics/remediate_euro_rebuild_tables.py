...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Rebuild the six unsafe EURO schemas through validated shadow tables. "
            "The default plan is read-only."
        )
    )
    parser.add_argument(
        "--stage",
...
def _write_report(output_dir, stage, suffix, result):
    ...
    output_path = output_dir / f"euro_rebuild_{stage}_{suffix}.json"
    payload = {
        "stage": stage,
        "suffix": suffix,
...
def print_plan(suffix):
    print("EURO rebuild remediation plan")
    ...
    for import_key in TARGET_IMPORT_KEYS:
        ...
        print(
            ...
            f"shadow={shadow_table_name(table_name, suffix)} | "
            f"retained={retained_table_name(table_name, suffix)}"
        )
    print(f"Build confirmation: {BUILD_CONFIRMATION}")
    print(f"Swap confirmation: {SWAP_CONFIRMATION}")
