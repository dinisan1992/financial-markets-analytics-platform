...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Atomically retain the current BLS table and promote the validated "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "audit_outputs" / "ecb_bls_swap_v081",
    )
    return parser
...
def _write_report(output_dir, payload):
    ...
    path = output_dir / "euro_bank_lending_survey_atomic_swap.json"
...
def main(argv=None):
    ...
    if args.confirm != SWAP_BLS_CONFIRMATION:
        raise ValueError(
    ...
    for path in (staging_dir, backup_dir, workspace_dir):
        if not path.is_dir():
            raise FileNotFoundError(f"Required BLS directory not found: {path}")
