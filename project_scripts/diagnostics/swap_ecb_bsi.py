...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Atomically retain the current BSI table and promote the validated "
            "official shadow. Automatic rollback is enabled."
        )
    )
...
def _write_report(output_dir, payload):
    ...
    path = output_dir / "euro_balance_sheet_items_atomic_swap.json"
...
def main(argv=None):
    ...
    if args.confirm != SWAP_BSI_CONFIRMATION:
        raise ValueError(
    ...
    verification_path, verification_sha256, verification_payload = (
        _verified_payload(
            ...
            "BSI post-build verification report",
        )
    )
    ...
    for path in (staging_dir, backup_dir, workspace_dir):
        if not path.is_dir():
            raise FileNotFoundError(f"Required BSI directory not found: {path}")
    ...
    print(f"Source-to-active valid: {result['source_to_active_audit']['valid']}")
