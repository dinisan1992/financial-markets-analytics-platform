...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Independently revalidate one built ECB shadow without changing "
            "the database or active CSV."
        )
    )
    parser.add_argument("import_key", choices=VERIFIABLE_IMPORT_KEYS)
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "audit_outputs" / "ecb_shadow_post_build_v084",
    )
    return parser
...
def _write_report(output_dir, payload):
    ...
    path = output_dir / (
        f"{payload['import_key'].lower()}_post_build_verification.json"
    )
...
def main(argv=None):
    ...
    print(f"Independent validation: {validation['valid']}")
