...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Atomically retain the current PCP table and promote the validated "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "audit_outputs" / "ecb_pcp_swap_v083",
    )
    return parser
...
def _write_report(output_dir, payload):
    ...
    path = output_dir / "euro_card_payments_atomic_swap.json"
...
def main(argv=None):
    ...
    if args.confirm != SWAP_PCP_CONFIRMATION:
        raise ValueError(
