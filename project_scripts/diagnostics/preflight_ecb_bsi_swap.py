...
from services.ecb_bsi_swap_service import preflight_bsi_swap
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Revalidate the complete BSI promotion evidence without changing "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "audit_outputs" / "ecb_bsi_swap_preflight_v086",
    )
    return parser
...
def _write_report(output_dir, payload):
    ...
    path = output_dir / "euro_balance_sheet_items_swap_preflight.json"
