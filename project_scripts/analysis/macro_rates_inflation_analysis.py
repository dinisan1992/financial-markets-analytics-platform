...
SELECTED_PAIR_KEY = "fed_funds_nasdaq"
...
RATES_INFLATION_MARKET_PAIRS = {
    "fed_funds_sp500": {
        "macro_asset": "FED_FUNDS_RATE",
        "market_asset": "SP500",
        "label": "Fed Funds Rate vs S&P 500",
        "description": "Compares Fed monetary policy with US equities."
    },

    "fed_funds_nasdaq": {
        "macro_asset": "FED_FUNDS_RATE",
        "market_asset": "NASDAQ100",
        "label": "Fed Funds Rate vs NASDAQ 100",
        "description": "Compares Fed rates with growth/technology assets."
    },

    "fed_funds_btc": {
        "macro_asset": "FED_FUNDS_RATE",
        "market_asset": "BTC",
        "label": "Fed Funds Rate vs BTC",
        "description": "Compares restrictive/expansionary monetary policy with Bitcoin."
    },

    "fed_funds_gold": {
        "macro_asset": "FED_FUNDS_RATE",
        "market_asset": "GOLD",
        "label": "Fed Funds Rate vs Gold",
        "description": "Compares Fed rates with gold as a reserve/safe-haven asset."
    },

    "fed_funds_dxy": {
        "macro_asset": "FED_FUNDS_RATE",
        "market_asset": "DXY",
        "label": "Fed Funds Rate vs DXY",
        "description": "Compares Fed rates with dollar strength."
    }
}
...
def calcular_rolling_correlations(df, macro_asset, market_asset):
    ...
    # For rates, use mostly absolute change.
    # Example: rate moves from 1% to 5% = +4 percentage points.
...
def gerar_summary_rates_inflation(df, macro_asset, market_asset, label):
    macro_cfg = MACRO_ASSETS[macro_asset]
...
def gerar_grafico_rolling_corr(df, macro_asset, market_asset, label):
    ...
    fig.update_layout(
        title=dict(
            text=f"Rolling Correlation - Macro Absolute Change vs Market Return | {label}",
            x=0.5,
            xanchor="center",
...
def export_resultados(df, summary_df, pair_key, macro_asset, market_asset):
    ...
    data_output = f"macro_rates_inflation_{safe_name}.csv"
    summary_output = f"macro_rates_inflation_summary_{safe_name}.csv"
...
def executar_par(pair_key, config, engine):
    ...
    print("\n" + "=" * 130)
    print(f"RATES MARKET ANALYSIS - {label}")
    print("=" * 130)
    ...
    summary_df = gerar_summary_rates_inflation(
        df=df,
        macro_asset=macro_asset,
        market_asset=market_asset,
        label=label
...
def main():
    print("\nA iniciar Macro Rates Analysis...")
    print(f"Run mode: {RUN_MODE}")
    print(f"Start date: {START_DATE}")
    print(f"End date: {END_DATE}")
    print(f"Pares configurados: {len(RATES_INFLATION_MARKET_PAIRS)}")
    ...
    if RUN_MODE == "single":
        if SELECTED_PAIR_KEY not in RATES_INFLATION_MARKET_PAIRS:
            raise ValueError(
                f"SELECTED_PAIR_KEY does not exist: {SELECTED_PAIR_KEY}"
            )

        config = RATES_INFLATION_MARKET_PAIRS[SELECTED_PAIR_KEY]
        ...
        for idx, (pair_key, config) in enumerate(RATES_INFLATION_MARKET_PAIRS.items(), start=1):
            print("\n" + "#" * 130)
            print(f"[{idx}/{len(RATES_INFLATION_MARKET_PAIRS)}] {pair_key}")
            print("#" * 130)
    ...
    if final_summaries:
        ...
        final_summary_df.to_csv(
            "macro_rates_inflation_summary_all.csv",
            index=False,
            sep=";",
            encoding="utf-8-sig"
        )
        ...
        print("macro_rates_inflation_summary_all.csv")

    print("\nMacro Rates Analysis completed.")
