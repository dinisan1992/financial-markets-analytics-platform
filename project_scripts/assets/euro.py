...
# CSV IMPORT PREVIEW
# =========================
def preview_csv_import_euro(csv_path, conn):
...
# =========================
def update_indicadores_euro_sql(df, cursor, conn):
    """
    Updates only existing legacy indicators in the euro_analysis.
    ...
    print("✅ Indicadores EURO updated no banco de data.")


# =========================
# UPDATE MANIPULATION FLAGS IN SQL
# =========================
def update_manipulacao_euro_sql(df, cursor, conn):
    update_sql = f"""
        UPDATE {TABLE_NAME}
        SET manipulation=%s
        WHERE snapped_at=%s
    ...
    print(f"✅ {len(flags_df)} flags de manipulation EURO updatesdas no SQL.")
...
# =========================
def build_parser():
    parser = argparse.ArgumentParser(description="Validate EURO market data safely.")
    parser.add_argument(
        "--dry-run-import",
        action="store_true",
...
def main(argv=None):
    ...
    try:
        ...
        if args.dry_run_import:
            preview_csv_import_euro(csv_path=CSV_PATH, conn=conn)
            return
        ...
        if df.empty:
            print("❌ No data available para EURO.")
            return
        ...
        if UPDATE_SQL:
            ...
            update_indicadores_euro_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )

            update_manipulacao_euro_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )
        ...
        # =========================
        if flags:
            print("\n⚠️ Possible manipulation signals detected for EURO:")

            for data, motivo in flags:
                print(f"{data}: {motivo}")

        else:
            print("\n✅ No clear manipulation signal identified no EURO.")

        print("✅ Processing EURO completed.")

    except Exception as e:
        print(f"❌ Error during processing EURO: {e}")
