...
# SETTINGS STOXX600
# =========================
CSV_PATH = ASSETS["STOXX600"]["csv_path"]
...
def parse_volume(vol):
    ...
    return pd.to_numeric(vol, errors="coerce") * multiplier


# =========================
# IMPORTAR CSV STOXX600
# =========================
def importar_csv_stoxx600(csv_path, conn, cursor):
    ...
    # VALIDAR COLUNAS
    # =========================
    required_cols = ["snapped_at", "price"]
    ...
    # VOLUME / TOTAL_VOLUME
    # =========================
    if "total_volume" in df_csv.columns:
        df_csv["total_volume"] = df_csv["total_volume"].apply(parse_volume)

    elif "volume" in df_csv.columns:
        df_csv["total_volume"] = df_csv["volume"].apply(parse_volume)

    else:
        print("⚠️ Volume column not found. Creating total_volume=0.")
        df_csv["total_volume"] = 0

    # =========================
    # CHANGE PERCENT OPCIONAL
    # =========================
    if "change_percent" in df_csv.columns:
        df_csv["change_percent"] = (
            df_csv["change_percent"]
            .astype(str)
            .str.replace("%", "", regex=False)
            .str.replace(",", "", regex=False)
        ...
        df_csv["change_percent"] = pd.to_numeric(
            df_csv["change_percent"],
            errors="coerce"
        )
...
# =========================
def update_indicadores_stoxx600_sql(df, cursor, conn):
    """
    Updates only existing legacy indicators in the stoxx600_analysis.
    ...
    print("✅ STOXX600 indicators updated in the database.")


# =========================
# UPDATE MANIPULATION FLAGS IN SQL
# =========================
def update_manipulacao_stoxx600_sql(df, cursor, conn):
    update_sql = f"""
        UPDATE {TABLE_NAME}
        SET manipulation=%s
        WHERE snapped_at=%s
    ...
    print(f"✅ {len(flags_df)} flags de manipulation STOXX600 updatesdas no SQL.")
...
def main():
    ...
    try:
        ...
        if df.empty:
            print("❌ No data available para STOXX600.")
            return
        ...
        if UPDATE_SQL:
            ...
            update_indicadores_stoxx600_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )

            update_manipulacao_stoxx600_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )
        ...
        # =========================
        if flags:
            print("\n⚠️ Possible manipulation signals detected for STOXX600:")

            for data, motivo in flags:
                print(f"{data}: {motivo}")

        else:
            print("\n✅ No clear manipulation signal identified no STOXX600.")

        print("✅ Processing STOXX600 completed.")

    except Exception as e:
        print(f"❌ Error during processing STOXX600: {e}")
