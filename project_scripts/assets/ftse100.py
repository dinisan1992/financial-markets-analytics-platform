...
# SETTINGS FTSE100
# =========================
CSV_PATH = ASSETS["FTSE100"]["csv_path"]
...
# =========================
# IMPORTAR CSV FTSE100
# =========================
def importar_csv_ftse100(csv_path, conn, cursor):
    ...
    # Fallback se o formato acima falhar em algumas rows
...
# =========================
def update_indicadores_ftse100_sql(df, cursor, conn):
    """
    Updates only existing legacy indicators in the ftse100_analysis.
    ...
    print("✅ Indicadores FTSE100 updated no banco de data.")


# =========================
# UPDATE MANIPULATION FLAGS IN SQL
# =========================
def update_manipulacao_ftse100_sql(df, cursor, conn):
    update_sql = f"""
        UPDATE {TABLE_NAME}
        SET manipulation=%s
        WHERE snapped_at=%s
    ...
    print(f"✅ {len(flags_df)} flags de manipulation FTSE100 updatesdas no SQL.")
...
def main():
    ...
    try:
        ...
        if df.empty:
            print("❌ No data available para FTSE100.")
            return
        ...
        if UPDATE_SQL:
            ...
            update_indicadores_ftse100_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )

            update_manipulacao_ftse100_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )
        ...
        # =========================
        if flags:
            print("\n⚠️ Possible manipulation signals detected for FTSE100:")

            for data, motivo in flags:
                print(f"{data}: {motivo}")

        else:
            print("\n✅ No clear manipulation signal identified no FTSE100.")

        print("✅ Processing FTSE100 completed.")

    except Exception as e:
        print(f"❌ Error during processing FTSE100: {e}")
