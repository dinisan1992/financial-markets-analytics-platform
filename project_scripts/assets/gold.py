...
# SETTINGS GOLD
# =========================
CSV_PATH = ASSETS["GOLD"]["csv_path"]

TABLE_NAME = "gold_analysis"
...
def parse_volume(valor):
    ...
    # Formato europeu:
    # 1.25K ou 1,25K dependendo da origem
    valor = valor.replace(".", "")
    valor = valor.replace(",", ".")
...
# IMPORTAR CSV GOLD
# =========================
def importar_csv_gold(csv_path, conn, cursor):
...
# =========================
def update_indicadores_gold_sql(df, cursor, conn):
    """
    Updates only existing legacy indicators in the gold_analysis.
    ...
    print("✅ Indicadores GOLD updated no banco de data.")


# =========================
# UPDATE MANIPULATION FLAGS IN SQL
# =========================
def update_manipulacao_gold_sql(df, cursor, conn):
    update_sql = f"""
        UPDATE {TABLE_NAME}
        SET manipulation=%s
        WHERE snapped_at=%s
    ...
    print(f"✅ {len(flags_df)} flags de manipulation GOLD updatesdas no SQL.")
...
def main():
    ...
    try:
        ...
        if df.empty:
            print("❌ No data available para GOLD.")
            return
        ...
        if UPDATE_SQL:
            ...
            update_indicadores_gold_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )

            update_manipulacao_gold_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )
        ...
        # =========================
        if flags:
            print("\n⚠️ Possible manipulation signals detected for GOLD:")

            for data, motivo in flags:
                print(f"{data}: {motivo}")

        else:
            print("\n✅ No clear manipulation signal identified no GOLD.")

        print("✅ Processing GOLD completed.")

    except Exception as e:
        print(f"❌ Error during processing GOLD: {e}")
