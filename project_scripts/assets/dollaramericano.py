...
# SETTINGS DXY
# =========================
CSV_PATH = ASSETS["DXY"]["csv_path"]

TABLE_NAME = "dxy_analysis"
...
# IMPORTAR CSV DXY
# =========================
def importar_csv_dxy(csv_path, conn, cursor):
...
# =========================
def update_indicadores_dxy_sql(df, cursor, conn):
    """
    Updates only existing legacy indicators in the dxy_analysis.
    ...
    print("✅ Indicadores DXY updated no banco de data.")


# =========================
# UPDATE MANIPULATION FLAGS IN SQL
# =========================
def update_manipulacao_dxy_sql(df, cursor, conn):
    update_sql = f"""
        UPDATE {TABLE_NAME}
        SET manipulation=%s
        WHERE snapped_at=%s
    ...
    print(f"✅ {len(flags_df)} flags de manipulation DXY updatesdas no SQL.")
...
def main():
    ...
    try:
        ...
        print("CSV import skipped. Use sync_market_data.py DXY --update-sql explicitly.")
        ...
        if df.empty:
            print("❌ No data available para DXY.")
            return
        ...
        if UPDATE_SQL:
            ...
            update_indicadores_dxy_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )

            update_manipulacao_dxy_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )
        ...
        # =========================
        if flags:
            print("\n⚠️ Possible manipulation signals detected for DXY:")

            for data, motivo in flags:
                print(f"{data}: {motivo}")

        else:
            print("\n✅ No clear manipulation signal identified no DXY.")

        print("✅ Processing DXY completed.")

    except Exception as e:
        print(f"❌ Error during processing DXY: {e}")
