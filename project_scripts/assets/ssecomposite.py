...
# =========================
# SETTINGS SSE COMPOSITE
# =========================
CSV_PATH = ASSETS["SSECOMPOSITE"]["csv_path"]
...
# CSV IMPORT PREVIEW
# =========================
def preview_csv_import_ssecomposite(csv_path, conn):
...
# =========================
def update_indicadores_ssecomposite_sql(df, cursor, conn):
    """
    Updates only existing legacy indicators in the ssecomposite_analysis.
    ...
    print("✅ SSE COMPOSITE indicators updated in the database.")


# =========================
# UPDATE MANIPULATION FLAGS IN SQL
# =========================
def update_manipulacao_ssecomposite_sql(df, cursor, conn):
    update_sql = f"""
        UPDATE {TABLE_NAME}
        SET manipulation=%s
        WHERE snapped_at=%s
    ...
    print(f"✅ {len(flags_df)} flags de manipulation SSE COMPOSITE updatesdas no SQL.")
...
# =========================
def build_parser():
    parser = argparse.ArgumentParser(description="Validate SSECOMPOSITE market data safely.")
    parser.add_argument(
        "--dry-run-import",
        action="store_true",
...
def main(argv=None):
    ...
    try:
        ...
        if args.dry_run_import:
            preview_csv_import_ssecomposite(csv_path=CSV_PATH, conn=conn)
            return
        ...
        if df.empty:
            print("❌ No data available para SSE COMPOSITE.")
            return
        ...
        if UPDATE_SQL:
            ...
            update_indicadores_ssecomposite_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )

            update_manipulacao_ssecomposite_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )
        ...
        # =========================
        if flags:
            print("\n⚠️ Possible manipulation signals detected for SSE COMPOSITE:")

            for data, motivo in flags:
                print(f"{data}: {motivo}")

        else:
            print("\n✅ No clear manipulation signal identified no SSE COMPOSITE.")

        print("✅ Processing SSE COMPOSITE completed.")

    except Exception as e:
        print(f"❌ Error during processing SSE COMPOSITE: {e}")
