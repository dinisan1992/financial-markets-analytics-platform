...
# SETTINGS LIBRA
# =========================
CSV_PATH = ASSETS["LIBRA"]["csv_path"]
...
# =========================
def update_indicadores_libra_sql(df, cursor, conn):
    """
    Updates only existing legacy indicators in the libra_analysis.
    ...
    print("✅ Indicadores LIBRA updated no banco de data.")


# =========================
# UPDATE MANIPULATION FLAGS IN SQL
# =========================
def update_manipulacao_libra_sql(df, cursor, conn):
    update_sql = f"""
        UPDATE {TABLE_NAME}
        SET manipulation=%s
        WHERE snapped_at=%s
    ...
    print(f"✅ {len(flags_df)} flags de manipulation LIBRA updatesdas no SQL.")
...
def main(argv=None):
    ...
    try:
        ...
        if df.empty:
            print("❌ No data available para LIBRA.")
            return
        ...
        if UPDATE_SQL:
            ...
            update_indicadores_libra_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )

            update_manipulacao_libra_sql(
                df=df,
                cursor=cursor,
                conn=conn
            )
        ...
        # =========================
        if flags:
            print("\n⚠️ Possible manipulation signals detected for LIBRA:")

            for data, motivo in flags:
                print(f"{data}: {motivo}")

        else:
            print("\n✅ No clear manipulation signal identified na LIBRA.")

        print("✅ Processing LIBRA completed.")

    except Exception as e:
        print(f"❌ Error during processing LIBRA: {e}")
