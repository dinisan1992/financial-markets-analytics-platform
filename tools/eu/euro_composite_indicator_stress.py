...
# CSV path
csv_path = EURO_SOURCE_DIR / "Composite Indicator of Systemic Stress.csv"
chunk_size = 5000
...
for chunk in pd.read_csv(csv_path, chunksize=chunk_size, low_memory=False):
    # Fix NaN / empty spaces
    chunk = chunk.map(lambda x: None if pd.isna(x) or str(x).strip().lower() == 'nan' else x)

    # Rename KEY column -> key_code to match the table
    if 'KEY' in chunk.columns:
        chunk = chunk.rename(columns={'KEY': 'key_code'})

    for _, row in chunk.iterrows():
        sql = """
            INSERT IGNORE INTO euro_composite_indicator_stress (
            ...
            VALUES (
                %(key_code)s, %(FREQ)s, %(REF_AREA)s, %(CURRENCY)s, %(PROVIDER_FM)s, %(INSTRUMENT_FM)s,
...
print("✅ All data imported successfully para 'euro_composite_indicator_stress'!")
