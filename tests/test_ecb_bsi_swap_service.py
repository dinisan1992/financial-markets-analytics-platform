...
from services.ecb_bsi_swap_service import (
    IMPORT_KEY,
    SWAP_BSI_CONFIRMATION,
    canonical_evidence,
    preflight_bsi_swap,
    swap_bsi_active,
...
def post_build_report(build=None):
    ...
    return {
        ...
        "independent_validation": complete_validation(),
...
class EcbBsiSwapServiceTests(unittest.TestCase):
    ...
    @patch("services.ecb_bsi_swap_service.inspect")
    @patch("services.ecb_bsi_swap_service.validate_existing_shadows")
    @patch("services.ecb_bsi_swap_service._validated_swap_context")
    ...
    @patch("services.ecb_bsi_swap_service.swap_validated_shadows")
    ...
    @patch("services.ecb_bsi_swap_service.shadow_table_evidence")
    @patch("services.ecb_bsi_swap_service.active_table_checkpoint")
    @patch("services.ecb_bsi_swap_service.validate_scoped_backup")
    @patch("services.ecb_bsi_swap_service._validate_pinned_file")
    @patch("services.ecb_bsi_swap_service.validate_readiness_report")
    @patch("services.ecb_bsi_swap_service.swap_validated_shadows")
    def test_changed_active_checkpoint_stops_before_swap_core(
        ...
        pinned_file.side_effect = [
            {"file_name": "Balance Sheet Items.csv", "sha256": "A"},
            {"file_name": "bsi.sql", "sha256": "B"},
        ]
        ...
        with self.assertRaisesRegex(ValueError, "checkpoint changed"):
            swap_bsi_active(
                Mock(),
                confirmation=SWAP_BSI_CONFIRMATION,
                readiness_payload={},
                build_payload=build,
    ...
    @patch("services.ecb_bsi_swap_service.final_active_evidence")
    @patch("services.ecb_bsi_swap_service.audit_euro_source_against_target")
    @patch("services.ecb_bsi_swap_service.shadow_table_evidence")
    @patch("services.ecb_bsi_swap_service.table_checkpoint")
    @patch("services.ecb_bsi_swap_service.active_table_checkpoint")
    @patch("services.ecb_bsi_swap_service.validate_scoped_backup")
    @patch("services.ecb_bsi_swap_service._validate_pinned_file")
    @patch("services.ecb_bsi_swap_service.validate_readiness_report")
    ...
    def test_swap_revalidates_and_preserves_retained_checkpoint(
        ...
        scoped_backup.return_value = {"sha256": "B", "path": Path("bsi.sql")}
        ...
        self.assertEqual(
            {IMPORT_KEY: Path("staging").resolve() / "Balance Sheet Items.csv"},
