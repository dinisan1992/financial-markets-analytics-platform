...
SWAP_VERSION = "v083"
VERIFICATION_VERSION = "v082"
SWAP_PCP_CONFIRMATION = "SWAP_EURO_CARD_PAYMENTS_V083_ACTIVE"
...
def _validate_complete_validation(validation, expected_rows, label):
    if validation.get("valid") is not True:
        raise ValueError(f"PCP {label} is not valid")
    exact_counts = (
        "source_rows",
        "shadow_rows",
    ...
    for field in exact_counts:
        if int(validation.get(field, -1)) != expected_rows:
            raise ValueError(f"PCP {label} {field} mismatch")
    zero_counts = (
        "null_business_keys",
    ...
    for field in zero_counts:
        if int(validation.get(field, -1)) != 0:
            raise ValueError(f"PCP {label} {field} is not zero")
...
def validate_build_report(
    ...
    *,
    ...
    if payload.get("stage") != "shadow_build":
        raise ValueError("PCP build report stage is not final")
    if payload.get("version") != PLAN_VERSION:
        raise ValueError("PCP build report version mismatch")
    if str(payload.get("import_key", "")).upper() != IMPORT_KEY:
        raise ValueError("PCP build report import mismatch")
    if payload.get("readiness_report_file") != readiness_report_file:
        raise ValueError("PCP build readiness-report file mismatch")
    if payload.get("readiness_report_sha256") != readiness_report_sha256:
        raise ValueError("PCP build readiness-report SHA-256 mismatch")
    planned_shadow = readiness_plan["planned_names"]["shadow_table"]
    if payload.get("shadow_table") != planned_shadow:
        raise ValueError("PCP build report shadow name mismatch")
    ...
    if payload.get("database_write_scope") != "versioned_shadow_table_only":
        raise ValueError("PCP build report write scope mismatch")
    if payload.get("database_write_performed") is not True:
        raise ValueError("PCP build report does not record its shadow write")
    if payload.get("active_table_changed") is not False:
        raise ValueError("PCP build report does not preserve the active table")
    if payload.get("active_tables_changed") is not False:
        raise ValueError("PCP build report records active-table changes")
    if payload.get("shadow_ready_for_swap_review") is not True:
        raise ValueError("PCP shadow is not ready for swap review")
    if payload.get("swap_authorized") is not False:
        raise ValueError("PCP build report already authorizes a swap")
    if payload.get("swap_performed") is not False:
        raise ValueError("PCP build report already records a swap")
    if payload.get("shadow_evidence", {}).get("hash_column_present") is not True:
        raise ValueError("PCP shadow technical hash is unavailable")
    _assert_equal(
        payload.get("source"),
        readiness_plan.get("candidate"),
        "PCP build source differs from readiness",
    )
    _assert_equal(
        payload.get("backup"),
        readiness_plan.get("backup"),
        "PCP build backup differs from readiness",
    )
    _assert_equal(
        payload.get("active_before"),
        payload.get("active_after"),
        "PCP active checkpoint changed during build",
    )
    return payload
...
def validate_post_build_report(
    ...
    *,
    ...
    if payload.get("stage") != "post_build_verification":
        raise ValueError("PCP post-build verification stage mismatch")
    if payload.get("version") != VERIFICATION_VERSION:
        raise ValueError("PCP post-build verification version mismatch")
    if str(payload.get("import_key", "")).upper() != IMPORT_KEY:
        raise ValueError("PCP post-build verification import mismatch")
    if payload.get("source_build_report_file") != build_report_file:
        raise ValueError("PCP post-build build-report file mismatch")
    if payload.get("source_build_report_sha256") != build_report_sha256:
        raise ValueError("PCP post-build build-report SHA-256 mismatch")
    if payload.get("database_write_performed") is not False:
        raise ValueError("PCP post-build report must be read-only")
    if payload.get("active_csv_write_performed") is not False:
        raise ValueError("PCP post-build report must preserve the active CSV")
    if payload.get("active_unchanged") is not True:
        raise ValueError("PCP post-build report does not preserve the active table")
    if payload.get("shadow_ready_for_review") is not True:
        raise ValueError("PCP post-build report does not approve shadow review")
    if payload.get("swap_authorized") is not False:
        raise ValueError("PCP post-build report already authorizes a swap")
    if payload.get("swap_performed") is not False:
        raise ValueError("PCP post-build report already records a swap")

    active_table = readiness_plan["active_table"]
    shadow_table = readiness_plan["planned_names"]["shadow_table"]
    expected_rows = int(readiness_plan["audit"]["source_rows"])
    if payload.get("tables") != [active_table, shadow_table]:
        raise ValueError("PCP post-build table inventory mismatch")
    row_counts = payload.get("row_counts", {})
    if int(row_counts.get(active_table, -1)) != int(
        readiness_plan["audit"]["target_rows"]
    ):
        raise ValueError("PCP post-build active row count mismatch")
    if int(row_counts.get(shadow_table, -1)) != expected_rows:
        raise ValueError("PCP post-build shadow row count mismatch")
    _assert_equal(
        payload.get("active_checkpoint"),
        build_payload.get("active_after"),
        "PCP post-build active checkpoint mismatch",
    )
    _assert_equal(
        payload.get("shadow_evidence"),
        build_payload.get("shadow_evidence"),
        "PCP post-build shadow evidence mismatch",
    )
    for field in ("initial_validation", "repeated_validation"):
        _assert_equal(
            payload.get(field),
            build_payload.get(field),
            f"PCP post-build {field} mismatch",
        )
        _validate_complete_validation(payload.get(field, {}), expected_rows, field)
    return payload
...
def final_active_evidence(engine, table_name, expected_rows):
    ...
    if evidence["rows"] != int(expected_rows):
        raise RuntimeError("Promoted PCP row count differs from source")
    if evidence["unique_business_keys"] != int(expected_rows):
        raise RuntimeError("Promoted PCP business keys are not unique")
    if evidence["null_business_keys"] != 0:
        raise RuntimeError("Promoted PCP contains null business keys")
    if "CHAR" not in evidence["time_period_type"]:
        raise RuntimeError("Promoted PCP time_period type is unsafe")
    if "DECIMAL" not in evidence["obs_value_type"]:
        raise RuntimeError("Promoted PCP obs_value type is not exact decimal")
    if primary_key != BUSINESS_KEY:
        raise RuntimeError("Promoted PCP primary key is incorrect")
    if evidence["hash_column_present"]:
        raise RuntimeError("Promoted PCP still contains the technical hash")
    return evidence
...
def _validated_swap_context(
    ...
    *,
    ...
    if build_payload.get("suffix") != suffix:
        raise ValueError("PCP build report suffix mismatch")
    ...
    if backup["sha256"] != backup_file["sha256"]:
        raise ValueError("PCP scoped backup verification changed")

    active_before = active_table_checkpoint(engine, IMPORT_KEY)
    _assert_equal(
        active_before,
        post_build_payload["active_checkpoint"],
        "Active PCP checkpoint changed after shadow validation",
    )
    shadow_table = readiness_plan["planned_names"]["shadow_table"]
    ...
    _assert_equal(
        ...
        "PCP shadow schema changed after validation",
    )
    return {
        "suffix": suffix,
...
def preflight_pcp_swap(
    ...
    *,
    ...
    if not validation.valid:
        raise RuntimeError("PCP shadow failed the preflight source validation")
    if validation.shadow_rows != context["expected_rows"]:
        raise RuntimeError("PCP preflight shadow row count differs from source")
    ...
    if retained != context["expected_retained_name"]:
        raise RuntimeError("PCP retained name differs from readiness plan")
    current_tables = set(inspect(engine).get_table_names())
    if context["active_table"] not in current_tables:
        raise RuntimeError("PCP active table is unavailable")
    if context["shadow_table"] not in current_tables:
        raise RuntimeError("PCP shadow table is unavailable")
    if retained in current_tables:
        raise RuntimeError("PCP retained table name already exists")
    if failed in current_tables:
        raise RuntimeError("PCP failed-table name already exists")
...
def swap_pcp_active(
    ...
    *,
    ...
    if confirmation != SWAP_PCP_CONFIRMATION:
        raise ValueError(
            f"--confirm must exactly match {SWAP_PCP_CONFIRMATION}"
        )
    context = _validated_swap_context(
        engine,
        readiness_payload=readiness_payload,
    ...
    def validate_promoted_state(*, engine, validations, retained_tables):
        ...
        if not validation.valid or validation.shadow_rows != expected_rows:
            raise RuntimeError("PCP shadow lost validation before swap")
        retained = retained_tables[0]
        if retained != expected_retained_name:
            raise RuntimeError("Retained PCP name differs from readiness plan")
        retained_checkpoint = table_checkpoint(engine, retained)
        _assert_equal(
            retained_checkpoint,
            expected_retained,
            "Retained PCP differs from pre-swap active",
        )
        promoted = shadow_table_evidence(engine, active_table)
        _assert_equal(
            promoted,
            shadow_before,
            "Promoted PCP differs from validated shadow schema",
        )
    ...
    def validate_final_state(*, engine, validations, retained_tables):
        ...
        _assert_equal(
            retained_checkpoint,
            expected_retained,
            "Retained PCP changed after promotion",
        )
        ...
        if not audit.valid:
            raise RuntimeError("Promoted PCP differs from official source")
